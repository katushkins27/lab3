...
public class Shorty extends Person implements ClothingWear, Sleeper {
    ...
    @Override
    public void setClothes(Clothing clothes) throws ClothingException{
        if (clothes == null){
            throw new ClothingException("Не могу надеть несуществующую одежду");
        }
        this.outfit = clothes;
    }
    ...
    /**
     * Реализация абстрактного метода, переопределяется
     * в классах наследниках - классы конкретных персонажей,
     * где дописана вариативность
      */
    public void baseAction() {
        speak("Я делаю обычное дело");
    }
     ...
     */
    public void getSick(HealthState newState) {
        this.healthState = newState;
        System.out.printf("%s простудился в %s и %s\n", name, location.getName(), healthState.getDescription());
