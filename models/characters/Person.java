...
public abstract class Person implements Speaker, StoryTeller, Mover, Recognizer {
    ...
    /**
     * Описываем состояние для понятного вывода для пользователя.
     * @see EmotionalState
     */
    private String getEmotionalStateDescription(){
        return switch (emotionalState) {
            case HAPPY -> "счастливый";
            case SAD -> "грустный";
            case AFRAID -> "испуганный";
            case CALM -> "спокойный";
            case CONFUSED -> "растерянный";
            default -> "чилловый";
        };
    }
    ...
    public void setEmotionalState(EmotionalState state){
        ...
        System.out.printf("%s %s\n", name, getEmotionalStateDescription());
    }
     ...
     */
    @Override
    public boolean recognizeChance(Person other, String context){
    ...
    public String toString() {
        return String.format("Персонаж{имя='%s', внимательность=%d, эмоции=%s, местоположение=%s}",
                name, attentiveness, emotionalState, location.getName());
    }
}
